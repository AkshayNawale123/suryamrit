...
const LokarthBadge: React.FC<LokarthBadgeProps> = ({
  ...
  const BadgeContent = (
    ...
    >
      <img src={lokarthLogo} alt="Lokarth Foundation" className={cn("w-auto object-contain", sizeClasses[size])} />
      <span className={cn("text-xs font-medium whitespace-nowrap", textClasses[variant])}>
        ...
        <TooltipContent className="max-w-xs p-4 bg-card border-primary/20">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Lokarth's mission is to make Vitamin D3 Sufficient India! We believe elite nutrition shouldn't be a
              luxury.
            </p>
            <p className="text-xs text-secondary font-medium italic">"Serve Together. Change Forever."</p>
          </div>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
